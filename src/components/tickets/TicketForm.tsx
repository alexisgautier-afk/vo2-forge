  ...
  return (
    <form onSubmit={handleSubmit} className="card space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="label-mono block mb-1.5">Ticket ID</label>
          <input
            value={id}
      ...
      <div>
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ticket title" className={inputCls} required />
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          <label className="label-mono block mb-1.5">Priority</label>
          <select value={priority} onChange={(e) => setPriority(e.target.value as TicketPriority)} className={selectCls}>
            {PRIORITY_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>
        <div>
          <label className="label-mono block mb-1.5">Assignee</label>
          <input value={assignee} onChange={(e) => setAssignee(e.target.value)} placeholder="Name" className={inputCls} />
        </div>
      </div>
